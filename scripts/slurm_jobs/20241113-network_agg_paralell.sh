#!/bin/bash
#SBATCH --job-name=net_agg_par_dynchunk    # Job name
#SBATCH --output=logs/net_agg_par_dynchunk_%j.log   # Output file (%j = job ID)
#SBATCH --error=logs/net_agg_par_dynchunk_%j.err    # Error file
...
#SBATCH --time=24:00:00             # Time limit hrs:min:sec for test run
...
srun $python_path turnstile_network_aggregation-parallel.py
