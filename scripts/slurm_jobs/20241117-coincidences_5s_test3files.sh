#!/bin/bash
#SBATCH --job-name=coincidences_5s_test3files    # Job name
#SBATCH --output=logs/coincidences_5s_test3files_%j.log   # Output file (%j = job ID)
#SBATCH --error=logs/coincidences_5s_test3files_%j.err    # Error file
#SBATCH --nodes=1                   # Run on a single node
#SBATCH --ntasks=1                  # Run a single task
#SBATCH --cpus-per-task=4          #
#SBATCH --mem=16G                   # Request 64GB RAM (adjust if needed)
...
srun $python_path parallel-coincidences.py --window 5 --test --sample-size 3
