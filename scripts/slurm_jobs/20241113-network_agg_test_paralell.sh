#!/bin/bash
#SBATCH --job-name=net_agg_test_par_dynchunk_100f    # Job name
#SBATCH --output=logs/net_agg_test_par_dynchunk_100f_%j.log   # Output file (%j = job ID)
#SBATCH --error=logs/net_agg_test_par_dynchunk_100f_%j.err    # Error file
#SBATCH --nodes=1                   # Run on a single node
#SBATCH --ntasks=1                  # Run a single task
#SBATCH --cpus-per-task=6          # Increased to handle 5 parallel processes (one per window) + 1 spare
#SBATCH --mem=32G                   # Increased memory (~15GB per process + overhead)
#SBATCH --time=00:30:00             # Time limit hrs:min:sec for test run
...
# Run the Python script with MPI for test
srun $python_path turnstile_network_aggregation-parallel.py --test --test-files 100
