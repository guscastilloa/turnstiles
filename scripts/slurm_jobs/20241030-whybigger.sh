#!/bin/bash
#SBATCH --job-name=compare_size    # Job name
#SBATCH --output=logs/compare_size_%j.log   # Output file (%j = job ID)
#SBATCH --error=logs/compare_size_%j.err    # Error file
#SBATCH --nodes=1                   # Run on a single node
#SBATCH --ntasks=1                  # Run a single task
#SBATCH --cpus-per-task=1          # Use 2 CPUs (one for I/O, one for processing)
...
srun $python_path 20241030_compare_size.py
