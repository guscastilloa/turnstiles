#!/bin/bash
#SBATCH --job-name=net_agg_window4s    # Job name
#SBATCH --output=logs/net_agg_window4s_%j.log   # Output file (%j = job ID)
#SBATCH --error=logs/net_agg_window4s_%j.err    # Error file
...
echo "Processing window size: 7 seconds"
...
srun $python_path turnstile_network_aggregation-parallel.py --window 7
